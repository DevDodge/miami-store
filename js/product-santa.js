/**
 * CLIMBING SANTA - PRODUCT PAGE JAVASCRIPT
 * Version 2.1.0 - FULLY FIXED
 */
...
const CONFIG = {
    ...
    TOAST_DURATION: 3000,
    LOCAL_STORAGE_KEY: 'miamiStoreSanta',
    VIEWERS_MIN: 600,
    VIEWERS_MAX: 4000,
    ORDERS_START: 251,
    ORDERS_INCREMENT_CHANCE: 0.3
};

// Product Data
const santaProduct = {
    id: 'climbing-santa-001',
    ...
    create() {
        ...
        this.overlay.innerHTML = `
            <div style="text-align: center;">
                ...
                <p style="color: #F2C94C; margin-top: 8px; font-size: 1rem;">
        ...
        style.textContent = `
            .christmas-loader {
                ...
                border-top-color: #D92E2E;
                border-right-color: #1E8A4C;
                border-radius: 50%;
...
class LiveStatsManager {
    constructor() {
        const stored = Utils.getLocalStorage('santaStats');
    ...
    saveStats() {
        Utils.setLocalStorage('santaStats', {
...
class ProductGallery {
    ...
    loadThumbnails() {
        ...
        this.images.forEach((imagePath, index) => {
            ...
            img.src = imagePath;
            img.alt = `بابا نويل المتسلق - صورة ${index + 1}`;
            img.loading = index === 0 ? 'eager' : 'lazy';
            img.onerror = function() {
                this.src = `https://via.placeholder.com/400x500/D92E2E/FFFFFF?text=صورة+${index + 1}`;
            };
...
function updateOrderSummary(quantity) {
    ...
    const itemPrice = santaProduct.price;
        ...
        phoneInput.addEventListener('input', function(e) {
            ...
            if (this.value.length === 11) {
                if (Utils.validatePhone(this.value)) {
                    this.style.borderColor = '#1E8A4C';
                } else {
                    this.style.borderColor = '#D92E2E';
                }
            }
        });
    }
...
async function handleFormSubmission() {
    ...
    const formData = {
        productName: santaProduct.name,
        productPrice: santaProduct.price,
        quantity: quantity,
...
function showSuccessMessage(data) {
    ...
    const orderNumber = 'SAN' + Date.now().toString().slice(-6);
...
function trackOrder(formData) {
    const orders = Utils.getLocalStorage('santaOrders') || [];
    orders.push({
        ...formData,
        timestamp: Date.now()
    });
    Utils.setLocalStorage('santaOrders', orders);

    if (typeof gtag !== 'undefined') {
        gtag('event', 'purchase', {
            transaction_id: 'SAN' + Date.now(),
            value: formData.totalAmount,
            currency: 'EGP',
            items: [{
                item_id: santaProduct.id,
...
function createConfetti() {
    const colors = ['#D92E2E', '#1E8A4C', '#F2C94C', '#FFFFFF'];
...
function toggleMusic() {
    ...
    if (!audioElement) {
        audioElement = new Audio('assets/christmas-music.mp3');
    ...
    if (musicPlaying) {
        ...
        audioElement.play().catch(e => {
            ...
            Toast.show('تأكد من السماح بتشغيل الصوت في المتصفح', 'info', 2000);
        });
        icon.textContent = '🔊';
    }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('%c🎅 بابا نويل المتسلق', 'font-size: 24px; font-weight: bold; color: #D92E2E;');
    console.log('%c🎄 Miami Store - Christmas Edition v2.1.0', 'font-size: 14px; color: #1E8A4C;');
...
console.log('✅ Santa Product Page Loaded Successfully!');