/**
 * DANCING SANTA - PRODUCT PAGE JAVASCRIPT
...
const CONFIG = {
    ...
    TOAST_DURATION: 3000,
    LOCAL_STORAGE_KEY: 'miamiStoreDancingSanta',

    // Dancing Santa Product Specific
    VIEWERS_MIN: 600,
    VIEWERS_MAX: 4000,
    ORDERS_START: 138,
    ORDERS_INCREMENT_CHANCE: 0.3 // 30% chance per update
};

// Product Data
const dancingSantaProduct = {
    id: 'dancing-santa-002',
    name: 'بابا نويل المهرج الراقص',
    price: 699,
    oldPrice: 1399,
    discount: 50
};
...
class LiveStatsManager {
    constructor() {
        const stored = Utils.getLocalStorage('dancingSantaStats');
    ...
    saveStats() {
        Utils.setLocalStorage('dancingSantaStats', {
...
class ProductGallery {
    ...
    loadThumbnails() {
        ...
        this.images.forEach((imagePath, index) => {
            ...
            img.src = imagePath;
            img.alt = `بابا نويل المهرج الراقص - صورة ${index + 1}`;
...
function updateOrderSummary(quantity) {
    ...
    const itemPrice = dancingSantaProduct.price;
...
async function handleFormSubmission() {
    ...
    const formData = {
        productName: dancingSantaProduct.name,
        productPrice: dancingSantaProduct.price,
        quantity: quantity,
        ...
        source: 'Dancing Santa Product Page'
    };
...
function showSuccessMessage(data) {
    ...
    const orderNumber = 'DSN' + Date.now().toString().slice(-6);
    ...
    // ✅ REMOVED AUTO-SCROLL - Let user see success where they are
    createConfetti();
}

function trackOrder(formData) {
    const orders = Utils.getLocalStorage('dancingSantaOrders') || [];
    orders.push({
        ...formData,
        timestamp: Date.now()
    });
    Utils.setLocalStorage('dancingSantaOrders', orders);

    if (typeof gtag !== 'undefined') {
        gtag('event', 'purchase', {
            transaction_id: 'DSN' + Date.now(),
            value: formData.totalAmount,
            currency: 'EGP',
            items: [{
                item_id: dancingSantaProduct.id,
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('%c🎅🤡 بابا نويل المهرج الراقص', 'font-size: 24px; font-weight: bold; color: #E63946;');
    console.log('%c🎄 Miami Store - Dancing Santa Edition', 'font-size: 14px; color: #2A9D8F;');
    ...
    initializeForm();
    // ❌ REMOVED: initializeSmoothScroll();
    updateQuantity(0);
...
// Global exports
window.DancingSantaProductPage = {
    config: CONFIG,
    utils: Utils,
    updateQuantity
};